...
async function getNFTBuyersInHour(contractAddress, tokenId, rpcEndpoint) {
...
console.log(transferEvents);
    ...
    const buyers = {};
     ...
     events.forEach( async event => {
        ...
        function getCurrentTimeBigInt() {
            const currentTimeMillis = Date.now();
            const currentTimeBigInt = BigInt(currentTimeMillis);
            return currentTimeBigInt;
          }
          
          const endTimestamp = getCurrentTimeBigInt();
          const hoursToMilliseconds = 1n * 60n * 60n * 1000n;
          const startTimestamp =getCurrentTimeBigInt() - hoursToMilliseconds;

        
          

        if (timestamp >= startTimestamp && timestamp <= endTimestamp && event.args.to !== ethers.constants.AddressZero) {
            const buyer = event.args.to;
            if (!buyers[buyer]) {
                buyers[buyer] = true;
            }
        }
...
(async () => {
    try {
        const buyers = await getNFTBuyersInHour('0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d', 9656, 'https://eth-mainnet.g.alchemy.com/v2/W34HfXlsUSKIWT7wFt1Jh64z5bbHk1In');
